...
if ($testIssues.Count -eq 0) {
    ...
    Write-Host "  [!]   Found $($testIssues.Count) test issues" -ForegroundColor Yellow
...
if ($rubbishFiles.Count -eq 0) {
    ...
    Write-Host "  [!]   Found $($rubbishFiles.Count) unnecessary root files" -ForegroundColor Yellow
...
if ($toolingIssues.Count -eq 0) {
    ...
    Write-Host "  [!]   $($toolingIssues.Count) tooling improvements available" -ForegroundColor Yellow
...
|----------|-------|--------|
| FastMCP 2.12+ | $($results.Scores.FastMCP)/10 | $(if($results.Scores.FastMCP -ge 8){""}else{"[FAIL] "}) |
| MCPB Packaging | $($results.Scores.MCPB)/10 | $(if($results.Scores.MCPB -ge 8){""}else{"[FAIL] "}) |
| CI/CD | $($results.Scores.CICD)/10 | $(if($results.Scores.CICD -ge 7){""}else{"[FAIL] "}) |
| Test Scaffold | $($results.Scores.Tests)/10 | $(if($results.Scores.Tests -ge 7){""}else{"[FAIL] "}) |
| Folder Structure | $($results.Scores.Structure)/10 | $(if($results.Scores.Structure -ge 8){""}else{"[FAIL] "}) |
| Documentation | $($results.Scores.Documentation)/10 | $(if($results.Scores.Documentation -ge 8){""}else{"[FAIL] "}) |
| Repo Cleanliness | $($results.Scores.Cleanliness)/10 | $(if($results.Scores.Cleanliness -ge 8){""}else{"[FAIL] "}) |
| Modern Tooling | $($results.Scores.Tooling)/10 | $(if($results.Scores.Tooling -ge 8){""}else{"[FAIL] "}) |
...
$(if($results.Scores.FastMCP -ge 9){" Fully compliant with FastMCP 2.12+ standards"}else{"[!]  Review tool decorators for description= parameters"})

### MCPB Packaging
**Score:** $($results.Scores.MCPB)/10
$(if($results.Scores.MCPB -ge 9){" Complete MCPB package structure"}else{"[!]  Missing some MCPB required files"})

### CI/CD Workflows
**Score:** $($results.Scores.CICD)/10
$(if($results.Scores.CICD -ge 8){" GitHub Actions configured"}else{"[!]  Missing CI/CD workflows"})

### Test Coverage
**Score:** $($results.Scores.Tests)/10
$(if($results.Scores.Tests -ge 8){" Test infrastructure in place"}else{"[!]  Improve test coverage"})

### Documentation
**Score:** $($results.Scores.Documentation)/10
$(if($results.Scores.Documentation -ge 8){" Minimum documentation present"}else{"[!]  Missing key documentation files"})

### Repository Cleanliness
**Score:** $($results.Scores.Cleanliness)/10
$(if($results.Scores.Cleanliness -ge 9){" Clean repository root"}else{"[!]  Clean up unnecessary root files"})
