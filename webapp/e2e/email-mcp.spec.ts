...
const AUTH = { Authorization: 'Basic ' + Buffer.from('sandra:vienna2026').toString('base64') };

test.describe('Email-MCP Webapp', () => {

    test('Dashboard loads with KPIs', async ({ page }) => {
        await page.goto('/');
        await expect(page.getByRole('heading', { name: 'Email Hub Dashboard' })).toBeVisible();
    ...
    test('Compose page loads with AI Improve and Expander', async ({ page }) => {
        ...
        await page.fill('textarea[placeholder*="Write your email"]', 'Test email body for AI');
    ...
    test('Settings page loads', async ({ page }) => {
        ...
        await expect(page.getByRole('heading', { name: 'Email Service Credentials' })).toBeVisible();
    });

    test('AI Chat page loads with workflow presets', async ({ page }) => {
        await page.goto('/chat');
        await expect(page.getByRole('heading', { name: 'Email AI Expert' })).toBeVisible();
    ...
    test('Mail Lab page loads', async ({ page }) => {
        ...
        await expect(page.getByRole('heading', { name: 'SMTP Server' })).toBeVisible();
    });
    ...
    test('Help page has 6 tabs', async ({ page }) => {
        ...
        await expect(page.getByRole('tab', { name: 'Email Systems' })).toBeVisible();
    ...
    test('Sidebar navigation works', async ({ page }) => {
        await page.goto('/');
        const navItems = ['Inbox', 'Search', 'Compose', 'AI Chat', 'Mail Lab', 'Services', 'Contacts'];
...
test.describe('REST API', () => {
    test('GET /api/status returns connected', async ({ request }) => {
        const resp = await request.get('http://localhost:10813/api/status', { headers: AUTH });
    ...
    test('GET /api/services returns service list', async ({ request }) => {
        const resp = await request.get('http://localhost:10813/api/services', { headers: AUTH });
        expect(resp.ok()).toBeTruthy();
        const data = await resp.json();
        expect(data).toHaveProperty('services');
    });

    test('POST /api/services/quick rejects invalid provider', async ({ request }) => {
        const resp = await request.post('http://localhost:10813/api/services/quick', {
            headers: AUTH,
            data: { provider: 'nonexistent', email: 'test@test.com', password: 'pass' },
        });
    ...
    test('GET /api/watcher/status returns without crashing', async ({ request }) => {
        const resp = await request.get('http://localhost:10813/api/watcher/status', { headers: AUTH });
        expect(resp.ok()).toBeTruthy();
        const data = await resp.json();
    ...
    test('POST /api/workflow love-letter returns text', async ({ request }) => {
        const resp = await request.post('http://localhost:10813/api/workflow', {
            headers: AUTH,
            data: { workflow: 'love-letter', recipient: 'Test', tone: 'sincere', mood: 'warm', format: 'text' },
        });
        expect(resp.ok()).toBeTruthy();
        const data = await resp.json();
