...
function Avatar({ name, email }: { name: string; email: string }) {
  ...
  const color =
    colors[
      Math.abs(
        (email || name).split("").reduce((a, c) => a + c.charCodeAt(0), 0),
      ) % colors.length
    ];
  return (
    <div
      className={`h-9 w-9 rounded-full ${color} flex items-center justify-center text-white text-sm font-medium shrink-0`}
    >
      {initial}
    </div>
  );
...
export function MailReader() {
  ...
  useEffect(() => {
    ...
    const newIds = [...currentIds].filter(
      (id) => !prevEmailIds.current.has(id),
    );
    if (
      newIds.length > 0 &&
  ...
  const handleArchive = async (e: React.MouseEvent, emailId: string) => {
    ...
    try {
      await fetchWithAuth(
        `/api/inbox/${encodeURIComponent(emailId)}?service=${encodeURIComponent(svc)}&folder=${folder}`,
        { method: "DELETE" },
      );
  ...
  const handleStar = async (_e: React.MouseEvent, emailId: string) => {
    setStarredIds((prev) => {
      const n = new Set(prev);
      n.has(emailId) ? n.delete(emailId) : n.add(emailId);
      return n;
    });
  };
  ...
  function normalizeSubject(subj: string): string {
    return (
      subj
        .replace(
          /^(?:\[.*?\]\s*)?(?:Re|Fwd|Aw|回复|RE|FWD|AW|回覆|Antw|Sv|VS):\s*/i,
          "",
        )
        .replace(/\[.*?\]/g, "")
        .trim()
        .toLowerCase() || subj.toLowerCase()
    );
  }
  ...
  const threadKeys = threaded
    ? [...threads.keys()].sort((a, b) => {
        const aDate = threads.get(a)?.[0]?.date || "";
        const bDate = threads.get(b)?.[0]?.date || "";
    ...
    try {
      ...
      if (selectedService === "unified") {
        ...
        url = searchQuery.trim()
          ? `/api/search?q=${encodeURIComponent(searchQuery)}&${params}`
          : `/api/inbox?${params}`;
      }
  ...
  const handleSelect = async (email: Email) => {
    ...
    try {
      const data = await fetchWithAuth(
        `/api/inbox/${encodeURIComponent(email.id)}?service=${encodeURIComponent(svc)}&folder=${folder}`,
      );
      setSelectedEmail(data);
      fetchWithAuth(`/api/inbox/${encodeURIComponent(email.id)}/mark-read`, {
        method: "POST",
        body: JSON.stringify({ service: svc, folder }),
      }).catch(() => {});
    } catch {
  ...
  const handleDownload = (emailId: string, att: Attachment) => {
    const svc = selectedEmail?._service || selectedService;
    window.open(
      `/api/inbox/${encodeURIComponent(emailId)}/attachment/${att.part_index}?service=${encodeURIComponent(svc)}&folder=${folder}`,
      "_blank",
    );
  };
  ...
  useEffect(() => {
    ...
    const onMouseMove = (e: MouseEvent) => {
      if (dragging.current)
        setListWidth(Math.max(280, Math.min(600, e.clientX)));
    };
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      ...
      <div className="flex flex-1 overflow-hidden border border-slate-800 rounded-lg bg-slate-950/30">
        {/* Email list */}
        <div
          className={`${showMobileList ? "flex" : "hidden"} md:flex flex-col border-r border-slate-800 overflow-hidden`}
                    ...
                    <div
                      ...
                      onClick={() =>
                        setExpandedThreads((prev) => {
                          const n = new Set(prev);
                          expanded ? n.delete(key) : n.add(key);
                          return n;
                        })
                      }
                    >
                      <div className="h-9 w-9 rounded-full bg-indigo-600 flex items-center justify-center text-white text-sm font-medium shrink-0">
                        {thread.length}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2">
        ...
        <div
          ref={resizeRef}
          className="w-1 hover:w-1.5 bg-slate-800 hover:bg-blue-500 cursor-col-resize transition-colors hidden md:block shrink-0"
        />

        {/* Email detail */}
        <div
          className={`${!showMobileList ? "flex" : "hidden"} md:flex flex-1 flex-col overflow-hidden`}
        >
          {detailLoading ? (
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              ...
              {selectedEmail.html_body ? (
                ...
                <pre className="text-sm text-slate-300 whitespace-pre-wrap font-sans leading-relaxed">
                  {selectedEmail.text_body || "(No body)"}
                </pre>
              )}
            </div>
          ) : (
