...
import { Plus, Trash2, Loader2, ArrowUp, ArrowDown, GripVertical, Filter } from "lucide-react";
import { fetchWithAuth } from "@/lib/api";
import { useToast } from "@/components/toast";

type Rule = { id: string; name: string; match_field: string; match_pattern: string; filter_action: string; filter_target: string; enabled: boolean; service: string };
...
export function Rules() {
    ...
    const [newRule, setNewRule] = useState({ name: "", match_field: "subject", match_pattern: "", filter_action: "mark_read", filter_target: "", service: "default" });
    const [saving, setSaving] = useState(false);
    const [deleting, setDeleting] = useState<string | null>(null);

    const loadRules = useCallback(async () => {
        setLoading(true);
        try { const d = await fetchWithAuth("/api/auto-rules"); setRules(d.rules || []); }
        catch { setRules([]); }
    ...
    const handleAdd = async () => {
        ...
        try {
            ...
            if (data.success) { toast("success", `Rule '${data.rule.name}' added`); setShowAdd(false); loadRules(); }
    ...
    const handleDelete = async (id: string) => {
        setDeleting(id);
        try { await fetchWithAuth(`/api/auto-rules/${id}`, { method: "DELETE" }); toast("success", "Rule deleted"); loadRules(); }
        catch (err: unknown) { toast("error", err instanceof Error ? err.message : "Delete failed"); }
        finally { setDeleting(null); }
    };

    const handleToggle = async (rule: Rule) => {
        try { await fetchWithAuth(`/api/auto-rules/${rule.id}`, { method: "PUT", body: JSON.stringify({ enabled: !rule.enabled }) }); loadRules(); }
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight text-white">Rules</h2>
                    <p className="text-slate-400">Email processing rules — actions triggered on matching messages</p>
                </div>
            ...
            {showAdd && (
                <Card className="border-blue-800 bg-blue-950/20">
                    <CardHeader><CardTitle className="text-white text-sm">New Rule</CardTitle></CardHeader>
                    <CardContent className="space-y-3">
                        <div className="grid gap-3 md:grid-cols-2">
                            <div><Label className="text-slate-300">Name</Label><Input className="bg-slate-900 border-slate-700 text-white mt-1" placeholder="e.g. Auto-archive invoices" value={newRule.name} onChange={(e) => setNewRule({ ...newRule, name: e.target.value })} /></div>
                            <div>
                            ...
                            <div>
                                ...
                                <select className="bg-slate-900 border border-slate-700 text-white text-sm rounded px-3 py-2 w-full mt-1" value={newRule.filter_action} onChange={(e) => setNewRule({ ...newRule, filter_action: e.target.value })}>
                        ...
                        <div className="flex gap-2">
                            <Button size="sm" className="bg-blue-600 hover:bg-blue-700" onClick={handleAdd} disabled={saving}>
                                {saving ? <Loader2 className="h-3 w-3 mr-1 animate-spin" /> : <Plus className="h-3 w-3 mr-1" />} Add Rule
                            </Button>
                        </div>
                    </CardContent>
                </Card>
            ...
            {loading ? (
                ...
                <div className="text-center py-12">
                    ...
                    <p className="text-slate-500 text-sm">No rules yet. Rules let you automatically process incoming emails.</p>
                </div>
            ) : (
                <div className="space-y-1">
                    {rules.map((rule, i) => {
                        ...
                        return (
                            <div key={rule.id} className="flex items-center gap-3 py-2.5 px-3 rounded bg-slate-950/50 border border-slate-800 hover:border-slate-700 transition-colors">
                                <GripVertical className="h-4 w-4 text-slate-600 shrink-0 cursor-grab" />
                                <div className={`h-2.5 w-2.5 rounded-full shrink-0 ${rule.enabled ? "bg-emerald-500" : "bg-slate-600"}`} />
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2">
                                        <p className="text-sm text-white font-medium">{rule.name}</p>
                                        {action && <span className={`text-xs font-medium ${action.color}`}>{action.label}</span>}
                                        {rule.filter_target && <span className="text-xs text-slate-500">→ {rule.filter_target}</span>}
                                    </div>
                                    <p className="text-xs text-slate-500 mt-0.5">
                                        <span className="font-mono text-blue-300">{rule.match_field}</span>
                                        <span className="text-slate-600"> ~/</span><span className="text-emerald-300">{rule.match_pattern}</span><span className="text-slate-600">/</span>
                                    </p>
                                </div>
                                ...
                                <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-500 hover:text-red-400" onClick={() => handleDelete(rule.id)} disabled={deleting === rule.id}>
