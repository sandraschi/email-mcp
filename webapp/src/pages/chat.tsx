...
import { Send, Bot, User, Loader2, BookOpen, Cpu, Heart, Sparkles, Frown, ThumbsUp, AlertTriangle, HeartHandshake, Star, Skull, ChevronDown, ChevronUp } from "lucide-react";
...
const RECIPIENTS = [
    "Prince Charming", "Princess", "Landlady", "Landlord",
    "My Cat", "The AI Overlord", "Pizza Delivery Person",
    "My Bank Account", "The Moon", "My Houseplants",
    "The WiFi Router", "My Future Self", "The Ceiling Fan",
    "Neighbor's Dog", "The 5AM Alarm Clock",
...
export function Chat() {
    ...
    const addMessage = (role: "user" | "bot", content: string) => {
        setMessages(prev => [...prev, { role, content, timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }]);
    };
    ...
    const handleSend = async (query?: string) => {
        ...
        setLoading(true);
        try {
            const data = await fetchWithAuth("/api/chat", { method: "POST", body: JSON.stringify({ query: q }) });
            addMessage("bot", data.response || "No response.");
        } catch (err: unknown) {
            addMessage("bot", `Error: ${err instanceof Error ? err.message : String(err)}`);
    ...
    const handleWorkflow = async (wf: WorkflowDef) => {
        ...
        try {
            const data = await fetchWithAuth("/api/workflow", {
                method: "POST",
                body: JSON.stringify({ workflow: wf.id, recipient, tone: workflowTone, mood: workflowMood, format: workflowFormat }),
            });
            if (data.success) {
            ...
            addMessage("bot", `Error: ${err instanceof Error ? err.message : String(err)}`);
        } finally { setExecutingWorkflow(false); setSelectedWorkflow(null); }
    };
        ...
        <div className="flex h-[calc(100vh-8rem)] flex-col space-y-4">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-slate-400">Natural language email management powered by your AI provider</p>
                </div>
                <div className="flex items-center gap-3 text-xs">
                    {skillLoaded && <span className="flex items-center gap-1 text-emerald-400"><BookOpen className="h-3 w-3" /> Skill</span>}
                    {providerInfo && <span className="flex items-center gap-1 text-slate-400"><Cpu className="h-3 w-3" /> {providerInfo}</span>}
                </div>
            </div>
            ...
            <Card className="flex-1 border-slate-800 bg-slate-950/50 flex flex-col overflow-hidden">
                ...
                <div className="border-t border-slate-800 bg-slate-900/30">
                    ...
                    >
                        <span className="flex items-center gap-1"><Sparkles className="h-3 w-3" /> Creative Workflows</span>
                        {showWorkflows ? <ChevronUp className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}
                    </button>
                    {showWorkflows && (
                                ...
                                {WORKFLOWS.map((wf) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            onClick={() => setSelectedWorkflow(isActive ? null : wf.id)}
                                        >
                            ...
                            {selectedWorkflow && (
                                <div className="flex gap-2 items-end flex-wrap pt-1">
                                    <div className="min-w-[160px]">
                                        <label className="text-xs text-slate-500 block mb-0.5">Recipient</label>
                                        <select
                                    ...
                                    <div className="min-w-[120px]">
                                        <label className="text-xs text-slate-500 block mb-0.5">Tone</label>
                                        <select
                                            className="bg-slate-900 border border-slate-700 text-white text-xs rounded px-2 py-1.5 w-full"
                                            value={workflowTone}
                                            onChange={(e) => setWorkflowTone(e.target.value)}
                                        >
                                            {["romantic", "sincere", "humble", "enthusiastic", "polite", "comedic", "dramatic", "professional"].map((t) => <option key={t}>{t}</option>)}
                                        </select>
                                    </div>
                                    <div className="min-w-[120px]">
                                        <label className="text-xs text-slate-500 block mb-0.5">Mood</label>
                                        <select
                                            className="bg-slate-900 border border-slate-700 text-white text-xs rounded px-2 py-1.5 w-full"
                                            value={workflowMood}
                                            onChange={(e) => setWorkflowMood(e.target.value)}
                                        >
                                            {["passionate", "warm", "gentle", "urgent", "melancholic", "absurd", "cheerful", "stoic"].map((m) => <option key={m}>{m}</option>)}
                                        </select>
                                    </div>
                                    <div className="min-w-[100px]">
                                        <label className="text-xs text-slate-500 block mb-0.5">Format</label>
                                        <select
                                            className="bg-slate-900 border border-slate-700 text-white text-xs rounded px-2 py-1.5 w-full"
                                    ...
                                    <Button
                                        ...
                                        onClick={() => handleWorkflow(WORKFLOWS.find(w => w.id === selectedWorkflow)!)}
                                        disabled={executingWorkflow}
                                    >
                                        {executingWorkflow ? <Loader2 className="h-3 w-3 mr-1 animate-spin" /> : <Sparkles className="h-3 w-3 mr-1" />}
                                        Generate
                                    </Button>
                                </div>
                            )}
                ...
                <div className="p-4 border-t border-slate-800 bg-slate-900/30">
                    <form className="flex gap-2" onSubmit={(e) => { e.preventDefault(); handleSend(); }}>
                        <input
                            ...
                            disabled={loading}
                        />
                        <Button type="submit" size="icon" className="bg-blue-600 hover:bg-blue-700 disabled:opacity-50" disabled={!input.trim() || loading}>
