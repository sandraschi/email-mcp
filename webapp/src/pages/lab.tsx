...
export function Lab() {
  ...
  const [selectedEmail, setSelectedEmail] = useState<LabEmailDetail | null>(
    null,
  );
  const [emailLoading, setEmailLoading] = useState(false);
  const [_pollInterval, _setPollInterval] = useState<ReturnType<
    typeof setInterval
  > | null>(null);
  ...
  const handleForward = async (emailId: string) => {
    if (!forwardTo.trim()) {
      toast("error", "Enter a forwarding address");
      return;
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400">
            Throwaway SMTP server for testing emails locally
          </p>
        </div>
      </div>
      ...
      <Card
        className={`border ${serverRunning ? "border-emerald-900/30 bg-emerald-950/10" : "border-slate-800 bg-slate-950/50"}`}
      >
        ...
        <CardContent className="text-xs text-slate-400">
          {serverRunning ? (
            <span className="flex items-center gap-2">
              ...
              Running on{" "}
              <code className="text-emerald-300">127.0.0.1:{serverPort}</code>
          ...
          <CardContent>
            <div className="flex gap-2 flex-wrap items-end">
              ...
              <Button
                ...
                disabled={generating || !serverRunning}
              >
                {generating ? (
                  <Loader2 className="h-3 w-3 mr-1 animate-spin" />
                ) : (
            ...
            {!serverRunning && (
              <p className="text-xs text-amber-500 mt-2">
                Start the SMTP server first to generate emails
              </p>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="flex gap-2 items-end">
              <div className="flex-1">
                ...
                <Input
                  className="bg-slate-900 border-slate-700 text-white mt-1 text-xs"
                      ...
                      {emailLoading ? (
                        ...
                        <div className="bg-slate-900/50 rounded-md p-3 space-y-2">
                          ...
                          <div className="flex gap-2 pt-1">
                            ...
                            >
                              <Forward className="h-3 w-3 mr-1" /> Forward to{" "}
                              {forwardTo || "..."}
                            </Button>
                          </div>
                        </div>
