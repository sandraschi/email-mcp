...
type Draft = { id: string; to: string; cc: string; subject: string; body: string; html?: string; service: string; updated_at: number };
type Provider = { id: string; name: string; available: boolean | null; models: string[] };
...
export function Compose() {
    ...
    useEffect(() => {
        fetchWithAuth("/api/services")
            .then((data) => {
                ...
                const list = Object.entries(svcMap).map(([name, info]: [string, any]) => ({ name, type: info.type }));
                setServices(list);
            })
            .catch(() => {});
        loadDrafts();
    ...
    const handleSaveDraft = async () => {
        ...
        setSaving(true);
        try {
            const payload: Record<string, unknown> = { to, cc, subject, body, service, ...(draftId ? { id: draftId } : {}), ...(htmlBody && useHtml ? { html: htmlBody } : {}) };
            const data = await fetchWithAuth(draftId ? `/api/drafts/${draftId}` : "/api/drafts", { method: draftId ? "PUT" : "POST", body: JSON.stringify(payload) });
            setDraftId(data.draft?.id || data.draft_id);
            toast("success", "Draft saved");
            loadDrafts();
        } catch (err: unknown) { toast("error", err instanceof Error ? err.message : "Save draft failed"); } finally { setSaving(false); }
    };
    ...
    const handleDeleteDraft = async (id: string) => {
        ...
        } catch (err: unknown) { toast("error", err instanceof Error ? err.message : "Delete draft failed"); }
    };

    const handleSend = async () => {
        if (!to.trim() || !subject.trim() || !body.trim()) { setResult({ ok: false, msg: "To, Subject, and Body are required." }); return; }
        setSending(true);
        setResult(null);
        try {
            ...
            if (data.success) {
                ...
                setTo(""); setCc(""); setBcc(""); setSubject(""); setBody(""); setHtmlBody(""); setDraftId(null);
                if (draftId) { try { await fetchWithAuth(`/api/drafts/${draftId}`, { method: "DELETE" }); } catch { /* ignore */ } }
            } else { setResult({ ok: false, msg: data.error || "Send failed." }); toast("error", data.error || "Send failed"); }
        } catch (err: unknown) { setResult({ ok: false, msg: err instanceof Error ? err.message : String(err) }); toast("error", err instanceof Error ? err.message : "Send failed"); }
    ...
    const suggestSubject = async () => {
        ...
        setSuggesting(true);
        try {
            const data = await fetchWithAuth("/api/chat", { method: "POST", body: JSON.stringify({ query: `Suggest 3 short email subject lines for this body:\n\n${body.slice(0, 500)}` }) });
    ...
    const handleImprove = async () => {
        ...
        try {
            const data = await fetchWithAuth("/api/improve", {
                method: "POST",
                body: JSON.stringify({ text: body, style: improveStyle, length: improveLength, mood: improveMood }),
            });
            if (data.success && data.response) {
                setBody(data.response);
                toast("success", `Improved: ${improveStyle}, ${improveMood}, ${improveLength}`);
            } else {
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight text-white">Compose Email</h2>
            ...
            {showDrafts && (
                <Card className="border-slate-800 bg-slate-950/50">
                    <CardHeader className="pb-2"><CardTitle className="text-white text-sm">Saved Drafts</CardTitle></CardHeader>
                    <CardContent>
                        {drafts.length === 0 ? <p className="text-slate-500 text-sm italic">No drafts</p> : (
                            <div className="space-y-1">
                                {drafts.map((d) => (
                                    <div key={d.id} className="flex items-center justify-between py-2 px-3 rounded hover:bg-slate-900/50 transition-colors">
                                        <button className="flex-1 text-left text-sm text-slate-300 hover:text-white truncate" onClick={() => loadDraft(d)}>
                                            <span className="font-medium">{d.subject || "(no subject)"}</span>
                                            <span className="text-slate-500 ml-2 text-xs">→ {d.to || "no recipient"}</span>
                                        </button>
                                        <Button variant="ghost" size="icon" className="h-6 w-6 text-slate-600 hover:text-red-400" onClick={() => handleDeleteDraft(d.id)}><Trash2 className="h-3 w-3" /></Button>
                                    </div>
                                ))}
                            </div>
                        )}
            ...
            <Card className="border-slate-800 bg-slate-950/50">
                ...
                <CardContent className="space-y-4">
                    <div className="flex items-center gap-3">
                        ...
                        <select className="bg-slate-900 border border-slate-700 text-white text-sm rounded px-3 py-1.5 w-64" value={service} onChange={(e) => setService(e.target.value)}>
                            <option value="default">{defaultServices ? `default (${defaultServices.type})` : "default"}</option>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <Input className="bg-slate-900 border-slate-700 text-white flex-1" placeholder="optional, comma-separated" value={cc} onChange={(e) => setCc(e.target.value)} />
                    </div>
                    <div className="flex items-center gap-3">
                        <Label className="text-slate-300 w-16 shrink-0">BCC</Label>
                        <Input className="bg-slate-900 border-slate-700 text-white flex-1" placeholder="optional, comma-separated" value={bcc} onChange={(e) => setBcc(e.target.value)} />
                    </div>
                    <div className="flex items-center gap-3">
                        <Label className="text-slate-300 w-16 shrink-0">Subject</Label>
                        <div className="flex flex-1 gap-2">
                            <Input className="bg-slate-900 border-slate-700 text-white flex-1" placeholder="Subject line" value={subject} onChange={(e) => setSubject(e.target.value)} />
                            <Button variant="outline" size="sm" className="border-slate-700 text-slate-300 hover:bg-slate-800 shrink-0" onClick={suggestSubject} disabled={!body.trim() || suggesting} title="AI subject suggestion">
                        ...
                        <div className="flex-1 space-y-2">
                            <div className="flex gap-2 items-center">
                                <label className="flex items-center gap-1 text-xs text-slate-400 cursor-pointer">
                                    <input type="checkbox" checked={useHtml} onChange={(e) => setUseHtml(e.target.checked)} className="accent-blue-500" />
                                    HTML
                                </label>
                            </div>
                            {useHtml ? (
                                <textarea className="w-full bg-slate-900 border border-slate-700 rounded-md px-3 py-2 text-sm text-white resize-y min-h-[200px] focus:outline-none focus:ring-1 focus:ring-blue-500 font-mono" placeholder="<h1>Title</h1>&#10;<p>Your HTML content...</p>" value={htmlBody} onChange={(e) => setHtmlBody(e.target.value)} />
                            ) : (
                                <textarea className="w-full bg-slate-900 border border-slate-700 rounded-md px-3 py-2 text-sm text-white resize-y min-h-[200px] focus:outline-none focus:ring-1 focus:ring-blue-500" placeholder="Write your email..." value={body} onChange={(e) => setBody(e.target.value)} />
                            )}
                        </div>
                    </div>
                        ...
                        <Card className="border-purple-900/30 bg-purple-950/10">
                            <CardHeader className="pb-2 pt-3">
                                <div className="flex items-center gap-2">
                                    ...
                                    <CardTitle className="text-white text-sm">AI Improve</CardTitle>
                                </div>
                            </CardHeader>
                            <CardContent className="space-y-3">
                                <div className="grid grid-cols-3 gap-3">
                                    <div>
                                        ...
                                        <select className="bg-slate-900 border border-slate-700 text-white text-xs rounded px-2 py-1.5 w-full mt-1" value={improveStyle} onChange={(e) => setImproveStyle(e.target.value)}>
                                            {STYLES.map((s) => (<option key={s.value} value={s.value}>{s.label}</option>))}
                                        </select>
                                    </div>
                                    <div>
                                        <Label className="text-slate-400 text-xs">Length</Label>
                                        <select className="bg-slate-900 border border-slate-700 text-white text-xs rounded px-2 py-1.5 w-full mt-1" value={improveLength} onChange={(e) => setImproveLength(e.target.value)}>
                                            {LENGTHS.map((s) => (<option key={s.value} value={s.value}>{s.label}</option>))}
                                        </select>
                                    </div>
                                    <div>
                                        <Label className="text-slate-400 text-xs">Mood</Label>
                                        <select className="bg-slate-900 border border-slate-700 text-white text-xs rounded px-2 py-1.5 w-full mt-1" value={improveMood} onChange={(e) => setImproveMood(e.target.value)}>
                                            {MOODS.map((s) => (<option key={s.value} value={s.value}>{s.label}</option>))}
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <select className="bg-slate-900 border border-slate-700 text-white text-xs rounded px-2 py-1" disabled>
                                            <option>From Settings</option>
                                        </select>
                                        <Button variant="ghost" size="icon" className="h-6 w-6 text-slate-500 hover:text-white" onClick={loadProviders} disabled={loadingProviders} title="Refresh providers">
                    ...
                    {result && (
                        <div className={`flex items-center gap-2 text-sm px-3 py-2 rounded border ${result.ok ? "text-emerald-400 border-emerald-900 bg-emerald-950/30" : "text-red-400 border-red-900 bg-red-950/30"}`}>
                    ...
                    <div className="flex justify-end gap-2 pt-2">
                        ...
                        <Button variant="outline" className="border-slate-700 text-slate-300 hover:bg-slate-800" onClick={handleSaveDraft} disabled={saving}>
                            {saving ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Save className="h-4 w-4 mr-2" />}
                            Save Draft
                        </Button>
                        <Button className="bg-blue-600 hover:bg-blue-700" onClick={handleSend} disabled={sending || !to.trim() || !subject.trim() || !body.trim()}>
