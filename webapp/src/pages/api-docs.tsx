...
const ENDPOINTS = [
    ...
    { method: "GET",  path: "/api/inbox",          desc: "Fetch inbox (service, folder, limit, unread_only)" },
    { method: "POST", path: "/api/send",           desc: "Send email {to, subject, body, service?}" },
    ...
    { method: "POST", path: "/api/llm/configure",  desc: "Update AI provider at runtime" },
        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-slate-400">FastAPI auto-generated docs for Email-MCP REST surface</p>
                </div>
                <div className="flex gap-2">
                    <Button
                        variant={view === "swagger" ? "default" : "outline"}
                        size="sm"
                        className={view === "swagger" ? "bg-blue-600 hover:bg-blue-700" : "border-slate-700 text-slate-300 hover:bg-slate-800"}
                    ...
                    <Button
                        ...
                        size="sm"
                        className={view === "redoc" ? "bg-blue-600 hover:bg-blue-700" : "border-slate-700 text-slate-300 hover:bg-slate-800"}
                    ...
                    <Button
                        ...
                        onClick={() => window.open(`${BACKEND_BASE}/${view === "redoc" ? "redoc" : "docs"}`, "_blank")}
                    >
            ...
            <Card className="border-slate-800 bg-slate-950/50">
                ...
                <CardContent>
                    <div className="grid gap-1.5">
                        {ENDPOINTS.map((ep) => (
                            <div key={ep.method + ep.path} className="flex items-center gap-3 py-1">
                                <span className={`text-xs font-mono px-1.5 py-0.5 rounded border w-14 text-center shrink-0 ${METHOD_COLOR[ep.method] || "text-slate-400 border-slate-700"}`}>
                                    {ep.method}
                                </span>
                                <code className="text-xs text-slate-300 font-mono w-52 shrink-0">{ep.path}</code>
            ...
            <p className="text-xs text-slate-600 text-center">
                If the iframe is blank, the backend may not be running on port {BACKEND_PORT}.{" "}
                <a href={`${BACKEND_BASE}/docs`} target="_blank" rel="noreferrer" className="text-blue-500 hover:underline">
                    Open directly →
                </a>
            </p>
        </div>
