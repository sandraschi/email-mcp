...
import { Mail, RefreshCw, Inbox as InboxIcon, Loader2, AlertCircle, Filter, Trash2, Search, Clock } from "lucide-react";
...
export function Inbox() {
    ...
    useEffect(() => {
        fetchWithAuth("/api/services")
            .then((data) => {
                ...
                const list: Service[] = Object.entries(svcMap).map(([name, info]: [string, any]) => ({
                    name,
                    type: info.type,
                }));
                setServices(list);
    ...
    const handleOpenEmail = (email: Email) => {
        navigate(`/email?id=${encodeURIComponent(email.id)}&service=${selectedService}&folder=${folder}`);
    };
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-slate-400">
                        {emails.length} message{emails.length !== 1 ? "s" : ""} in {folder} via{" "}
                        <span className="text-blue-400">{selectedService}</span>
                    </p>
                </div>
                <div className="flex gap-2 items-center">
                    ...
                    >
                        <Clock className={`h-3.5 w-3.5 mr-1 ${autoRefresh ? "text-emerald-400" : "text-slate-600"}`} />
                    ...
                    {showFilters && (
                        <div className="flex flex-wrap gap-3 mt-3 pt-3 border-t border-slate-800">
                            ...
                            <Input
                                ...
                                onKeyDown={(e) => e.key === "Enter" && fetchEmails()}
                            />
                            <Button size="sm" className="bg-blue-600 hover:bg-blue-700" onClick={fetchEmails}>
                                Apply
                            </Button>
                        </div>
                    )}
            ...
            <Card className="border-slate-800 bg-slate-950/50">
                <CardHeader className="pb-2 flex flex-row items-center justify-between">
                    ...
                    <Button
                        ...
                        onClick={() => navigate(`/search?service=${selectedService}&folder=${folder}`)}
                    >
                    ...
                    {emails.map((email, i) => (
                        ...
                        >
                            <div className="mt-0.5 p-1.5 bg-slate-900 rounded shrink-0">
                                <Mail className={`h-3.5 w-3.5 ${email.read ? "text-slate-500" : "text-blue-400"}`} />
                            </div>
                            <div className="flex-1 min-w-0">
                                <p className={`text-sm truncate ${email.read ? "text-slate-300" : "text-white font-medium"}`}>
