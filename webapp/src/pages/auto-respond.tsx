...
export function AutoRespond() {
  ...
  const handleApprove = async (id: string) => {
    try {
      const data = await fetchWithAuth(`/api/auto-pending/${id}/approve`, {
        method: "POST",
      });
      if (data.success) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400">
            Rule-based and AI-powered email auto-reply
          </p>
        </div>
            ...
            <Card className="border-blue-800 bg-blue-950/20">
              ...
              <CardContent className="space-y-3">
                <div className="grid gap-3 md:grid-cols-2">
                  ...
                  <div className="flex gap-4 items-end pb-2">
                    ...
                    <label className="flex items-center gap-1 text-xs text-slate-300 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={newRule.response_mode === "spoof"}
                        onChange={(e) =>
                          setNewRule({
                            ...newRule,
                            response_mode: e.target.checked
                              ? "spoof"
                              : "normal",
                          })
                        }
                      ...
                      />{" "}
                      Spoof mode (hilarious replies to scammers)
                    </label>
                    <label className="flex items-center gap-1 text-xs text-slate-300 cursor-pointer">
                      <input
                        type="checkbox"
                        ...
                        disabled={
                          !newRule.use_ai && newRule.response_mode !== "spoof"
                        }
                      />{" "}
                ...
                {newRule.response_mode === "spoof" && (
                  <div className="grid gap-2 mt-1">
                    ...
                    <select
                      ...
                      value={newRule.spoof_tone}
                      onChange={(e) =>
                        setNewRule({ ...newRule, spoof_tone: e.target.value })
                      }
                    >
                      <option value="mock-stupid">
                        Mock Stupid (gullible)
                      </option>
                      <option value="irate">Irate (screaming)</option>
                      <option value="absurd">Absurd (sentient toaster)</option>
                      <option value="polite-but-confused">
                        Polite but Confused (British)
                      </option>
                    </select>
                    ...
                    <select
                      className="bg-slate-900 border border-slate-700 text-white text-xs rounded px-2 py-1.5 w-full"
                      value={newRule.spam_action}
                      onChange={(e) =>
                        setNewRule({ ...newRule, spam_action: e.target.value })
                      }
                    >
                      <option value="ignore">Ignore spam (don't reply)</option>
                      <option value="spoof">
                        Spoof spam (reply hilariously)
                      </option>
                    </select>
                  </div>
                )}
                {newRule.response_mode === "spoof" ? (
                  <div>
                    <Label className="text-slate-300">
                      AI Prompt (optional — leave blank for default)
                    </Label>
                    <textarea
                      className="w-full bg-slate-900 border border-slate-700 rounded-md px-3 py-2 text-sm text-white resize-y min-h-[60px] mt-1"
                      placeholder="Reply politely saying I'm out of office until Monday"
                      value={newRule.ai_prompt}
                      onChange={(e) =>
                        setNewRule({ ...newRule, ai_prompt: e.target.value })
                      }
                    />
                  </div>
                ) : (
                  <>
                    <div>
                      <Label className="text-slate-300">
                        Reply Subject (leave blank to use Re: original)
                      </Label>
                      <Input
                        className="bg-slate-900 border-slate-700 text-white mt-1"
                        value={newRule.reply_subject}
                        onChange={(e) =>
                          setNewRule({
                            ...newRule,
                    ...
                    <div>
                      <Label className="text-slate-300">Reply Body</Label>
                      <textarea
                        className="w-full bg-slate-900 border border-slate-700 rounded-md px-3 py-2 text-sm text-white resize-y min-h-[80px] mt-1"
                        value={newRule.reply_body}
                        onChange={(e) =>
                          setNewRule({ ...newRule, reply_body: e.target.value })
                        }
                      />
                    </div>
                  </>
          ...
          {rules.length === 0 ? (
            ...
            <div className="space-y-2">
              {rules.map((r) => (
                <div
                  key={r.id}
                  className="flex items-center gap-3 py-3 px-3 rounded bg-slate-950/50 border border-slate-800 hover:bg-slate-900/30"
                >
                  <div
                    className={`h-2 w-2 rounded-full ${r.enabled ? "bg-emerald-500" : "bg-slate-600"}`}
                  />
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500 truncate">
                      ...
                      {r.response_mode === "spoof" ? (
                        <>
                          <Skull className="h-3 w-3 inline text-red-400" />{" "}
                          spoof ({r.spoof_tone})
                        </>
                      ) : r.use_ai ? (
                        <>
              ...
              <Card key={p.id} className="border-amber-900/30 bg-amber-950/10">
                ...
                <CardContent className="space-y-2">
                  <div className="text-xs text-slate-400 bg-slate-900/50 rounded p-2 max-h-[80px] overflow-y-auto">
                    {p.email_body?.slice(0, 300)}
                  </div>
                  <div className="h-px bg-slate-800" />
                  <div>
                    <p className="text-xs text-emerald-400 font-medium">
                      Reply: {p.reply_subject}
                    </p>
                    <p className="text-xs text-slate-300 bg-slate-900/50 rounded p-2 mt-1">
                      {p.reply_body?.slice(0, 500)}
                    </p>
                  </div>
