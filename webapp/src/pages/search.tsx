...
export function SearchPage() {
    ...
    const [service, setService] = useState(searchParams.get("service") || "default");
    ...
    const handleSearch = async () => {
        ...
        setError(null);
        try {
            const params = new URLSearchParams({ q: query, service, folder, limit: "50" });
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center gap-4">
                <Button variant="outline" size="sm" className="border-slate-700 text-slate-300 hover:bg-slate-800" onClick={() => navigate("/inbox")}>
                    <ArrowLeft className="h-4 w-4 mr-1" /> Inbox
                </Button>
                <div>
                    <h2 className="text-2xl font-bold tracking-tight text-white">Search Emails</h2>
                ...
                <CardContent className="pt-4 pb-4">
                    <div className="flex gap-3 flex-wrap items-end">
                        ...
                        <div>
                            ...
                            <select className="bg-slate-900 border border-slate-700 text-white text-sm rounded px-2 py-1.5" value={service} onChange={(e) => setService(e.target.value)}>
                                <option value="default">default</option>
                            </select>
                        </div>
                        <div>
                            <label className="text-xs text-slate-400 block mb-1">Folder</label>
                            <select className="bg-slate-900 border border-slate-700 text-white text-sm rounded px-2 py-1.5" value={folder} onChange={(e) => setFolder(e.target.value)}>
                                {["INBOX", "Sent", "Drafts", "Trash", "Spam"].map((f) => <option key={f}>{f}</option>)}
                            </select>
                        </div>
                        <Button className="bg-blue-600 hover:bg-blue-700" onClick={handleSearch} disabled={loading || !query.trim()}>
                ...
                <Card className="border-slate-800 bg-slate-950/50">
                    ...
                    <CardContent>
                        ...
                        {!loading && !error && results.length === 0 && (
                            <p className="text-slate-500 text-sm italic py-8 text-center">No results found.</p>
                        )}
                        {results.map((email, i) => (
                            <div
                                key={email.id || i}
                                ...
                                onClick={() => navigate(`/email?id=${encodeURIComponent(email.id)}&service=${service}&folder=${folder}`)}
                            >
                                <div className="mt-0.5 p-1.5 bg-slate-900 rounded shrink-0">
                                    <Mail className="h-3.5 w-3.5 text-blue-400" />
                                </div>
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm truncate text-white font-medium">{email.subject || "(No Subject)"}</p>
